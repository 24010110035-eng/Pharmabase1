...
class Transaksi_model extends CI_Model {
    ...
    public function proses_penjualan($kasir_id, $pelanggan_id, $items, $metode_pembayaran = 'tunai')
    {
        if (empty($items)) {
            return false;
        }
        ...
        // Susun no_nota urut berdasarkan id asli (dijamin unik, tidak perlu acak lagi).
        $no_nota = 'TRX-' . date('Ymd') . '-' . str_pad($transaksi_id, 6, '0', STR_PAD_LEFT);
        $this->db->where('id', $transaksi_id)->update($this->table, array('no_nota' => $no_nota));
    ...
    public function get_history($pelanggan_id = null, $dari = null, $sampai = null, $limit = 200)
    {
        $this->db->select('t.*, u.nama_lengkap AS nama_kasir, pu.nama_lengkap AS nama_pelanggan,
                            (t.kasir_id = p.user_id) AS is_self_checkout')
                  ->from($this->table . ' t')
    ...
    {
        return $this->db->select('t.*, u.nama_lengkap AS nama_kasir, pu.nama_lengkap AS nama_pelanggan,
                                   (t.kasir_id = p.user_id) AS is_self_checkout')
                          ->from($this->table . ' t')
                          ->join('users u', 'u.id = t.kasir_id')
                          ->join('pelanggan p', 'p.id = t.pelanggan_id', 'left')
