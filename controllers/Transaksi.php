...
class Transaksi extends MY_Controller {
	...
	public function proses()
	{
		...
		$metode_pembayaran = $this->input->post('metode_pembayaran') ?: 'tunai';

		if (!in_array($metode_pembayaran, array('tunai', 'transfer', 'qris', 'kartu'), true)) {
			$metode_pembayaran = 'tunai';
		}

		$transaksi_id = $this->Transaksi_model->proses_penjualan($this->user['id'], $pelanggan_id, array_values($cart), $metode_pembayaran);
	...
	public function checkout_pelanggan()
	{
		...
		$transaksi_id = $this->Transaksi_model->proses_penjualan($this->user['id'], $pelanggan->id, array_values($cart), $metode_pembayaran);
	...
	public function struk($id)
	{
		...
		// Batasi akses: pelanggan hanya boleh lihat struk miliknya sendiri.
		if ($this->user['role'] === 'pelanggan') {
			$pelanggan = $this->Pelanggan_model->get_by_user_id($this->user['id']);
			if (!$pelanggan || (int) $transaksi->pelanggan_id !== (int) $pelanggan->id) {
				show_error('Anda tidak memiliki akses ke struk ini.', 403, 'Akses Ditolak');
				return;
			}
		}

		$data['transaksi'] = $transaksi;
		$data['items']     = $this->Transaksi_model->get_items($id);

		// Halaman struk berdiri sendiri (tanpa sidebar/navbar), supaya rapi saat dicetak.
